...
in the root directory to generate the container. This will generate a single container on docker containing the 3 seperate images. Make sure to have your environment variables set for this.

Another option for Joint Deployment is to use the docker images that are generated by our github action. The Build and Push Hourly Container action will generate the server and client images automatically and publish them to ghcr.

### Seperate Deployment

If you wish to deploy the images to seperate containers/servers, you can use the images generated from the github actions. These are currently build to deploy to caprover but can easily be configured to deploy to any source of your choosing. There is additionally github actions for deploying to heroku.
